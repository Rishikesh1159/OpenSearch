...
public class SegmentReplicationRelocationIT extends OpenSearchIntegTestCase {
...
//    }
    ...
    public void testSimpleRelocation() throws Exception{
        ...
        logger.info("--> Done relocating the shard from node1 to node2");
        waitForReplicaUpdate();
...
//        int numberOfNodes = numberOfReplicas == 0 ? 2 : 3;
        ...
        logger.info("--> creating test index ...");
        prepareCreate("test", Settings.builder().put("index.number_of_shards", 1)
            .put("index.number_of_replicas", numberOfReplicas).put(IndexMetadata.SETTING_REPLICATION_TYPE, ReplicationType.SEGMENT)).get();
...
//        ).get();
        ...
        System.out.println("Node 0 shard routing: "+node0ShardRouting);
        System.out.println("Node 1 shard routing: "+node1ShardRouting);
        System.out.println("Node 2 shard routing: "+node2ShardRouting);
        ...
        for (int i = 0; i < numberOfRelocations; i++) {
            ...
            List<IndexRequestBuilder> builders3 = new ArrayList<>();
            for (int numDocs = randomIntBetween(10, 30); numDocs > 0; numDocs--) {
                builders3.add(client().prepareIndex("test").setSource("{}", XContentType.JSON));
            }
            ...
            System.out.println("Node 0 shard routing: "+afterNode0ShardRouting);
            System.out.println("Node 1 shard routing: "+afterNode1ShardRouting);
            System.out.println("Node 2 shard routing: "+afterNode2ShardRouting);

            logger.debug("--> verifying all searches return the same number of docs");

            IndexShard indexShard1 = getIndexShard(nodes[toNode]);


//            waitForReplicaUpdate(nodes);
//            indexShard1.refresh("test");
//            waitForReplicaUpdate(nodes);
//            System.out.println("got here 2");
//            Thread.sleep(1000);

//            System.out.println("in node "+nodes[0]+" res is: "+client(nodes[0]).prepareSearch("test").setPreference("_local").setSize(0).get().getHits().getTotalHits().value);
//
//            System.out.println("in node "+nodes[1]+" res is: "+client(nodes[1]).prepareSearch("test").setPreference("_local").setSize(0).get().getHits().getTotalHits().value);
//
//            System.out.println("in node "+nodes[2]+" res is: "+client(nodes[2]).prepareSearch("test").setPreference("_local").setSize(0).get().getHits().getTotalHits().value);

//            IndexShard indexShard0 = getIndexShard(nodes[0]);
//            System.out.println("shard in node 0 is: "+indexShard0);
//
//            IndexShard indexShard1 = getIndexShard(nodes[1]);
//            System.out.println("shard in node 1 is: "+indexShard1);
//
//            IndexShard indexShard2 = getIndexShard(nodes[2]);
//            System.out.println("shard in node 2 is: "+indexShard2);
//            indexRandom(true, true, builders3);
//
//            System.out.println("in node "+nodes[0]+" res is: "+client(nodes[0]).prepareSearch("test").setPreference("_local").setSize(0).get().getHits().getTotalHits().value);
//
//            System.out.println("in node "+nodes[1]+" res is: "+client(nodes[1]).prepareSearch("test").setPreference("_local").setSize(0).get().getHits().getTotalHits().value);
...
//            IndicesService indicesService = internalCluster().getInstance(IndicesService.class, nodes[1]);
//            System.out.println("index is "+ indicesService.hasIndex(index));
//            IndexService indexService = indicesService.indexServiceSafe(index);
//            System.out.println("number of shards is: "+indexService.numberOfShards());
//            Thread.sleep(1000);
            assertBusy(()-> {
            ...
            for (ShardSegments[] replicationGroupSegments : segmentsByIndex) {
                ...
                if (primaryShardSegments.getSegments().isEmpty() == false) {
                    ...
                    for (ShardSegments shardSegments : replicaShardSegments) {
                        ...
                        System.out.println("Replica is caught up yet "+isReplicaCaughtUpToPrimary);
                        System.out.println("Replica is caught up yet "+shardSegments.getSegments().stream().map(e -> e.getNumDocs()).reduce(0, (x, y) -> x + y));
    ...
    public void testRelocateWhileContinuouslyIndexingAndWaitingForRefresh() throws Exception {
        ...
        prepareCreate(
            "test",
            Settings.builder().put("index.number_of_shards", 1).put(IndexMetadata.SETTING_REPLICATION_TYPE, ReplicationType.SEGMENT).put("index.number_of_replicas", 0).put("index.refresh_interval", -1) // we
            // want
            // to
            // control
            // refreshes
